...
try:
    ...
    with mpPose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5, static_image_mode=False, model_complexity=0) as pose:
        while cap.isOpened():
            ...
            scaledImage = cv2.resize(imgRGB, (0,0), fx=0.5, fy=0.5)
            results = pose.process(scaledImage)
            current_time = time.time()
