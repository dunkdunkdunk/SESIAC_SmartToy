...
PLAY_AGAIN_SOUND = pygame.mixer.Sound("sound/Take ball back.wav")
...
try:
    while cap.isOpened():
        ...
        if arduino.in_waiting > 0:
            incoming_byte = arduino.read()
            ard_decoded = incoming_byte.decode('utf-8')
            # "e" ending, "b" signal bumper
            print("decode arduino", ard_decoded)
            if ard_decoded == "e":
                lcd.write("l".encode())
        ...
        if not result.pose_landmarks:
            if current_time - last_sound_time["exit"] > SOUND_COOLDOWN:
                play_sound(EXIT_FRAME_SOUND, "exit")
                last_sound_time["exit"] = current_time
            continue
        ...
        if nose_x < 0.1 or nose_x > 0.9:
            ...
            if current_time - last_sound_time["left"] > SOUND_COOLDOWN:
                play_sound(MOVE_LEFT_SOUND, "left")
                last_sound_time["left"] = current_time
            continue
        elif nose_x > 0.7:
            if current_time - last_sound_time["right"] > SOUND_COOLDOWN:
                play_sound(MOVE_RIGHT_SOUND, "right")
                last_sound_time["right"] = current_time
            continue
        ...
        if not play:
            ...
            # if GPIO.input(END_IR_PIN) == GPIO.LOW:
            if ard_decoded == "e":
